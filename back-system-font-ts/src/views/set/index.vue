...
const swiperUrl = ref(`${import.meta.env.VITE_API_BASEURL}/set/uploadSwiper`)
	...
	const handleAvatarSuccess = (
		...
		if (response.status == 0) {
			...
			(async () => {
				// console.log(localStorage.getItem('account') as unknown as number, response.onlyId, response.url)
				// await bind( localStorage.getItem('account') as unknown as number, response.onlyId, response.url)
				await bind( userStore.account as unknown as number, response.onlyId, response.url)
				// console.log(res)
			})()
		} else {
	...
	const saveName = async () => {
		const res = await changeName(userData.name, localStorage.getItem('id') as unknown as number)
		if (res.status == 0) {
			ElMessage({
				message: '修改成功',
				type: 'success',
	...
	// 保存性别
	const saveSex = async () => {
		const res = await changeSex(userData.sex, localStorage.getItem('id') as unknown as number)
		if (res.status == 0) {
			ElMessage({
				message: '修改成功',
				type: 'success',
