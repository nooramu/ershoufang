...
exports.getCategoryAndNumber = (req, res) => {
	...
	// 	})
	// }
	// 通过循环类别数组里面每一个类别获取对应的数量
	    async function getAll() {
	        const category = await CategoryArr();
			const sql = 'SELECT product_category, COUNT(*) AS count FROM product GROUP BY product_category;'
			db.query(sql,(err,result)=>{
				const counts = result.map(result => result.count);
				res.send({
					category:category,
					price:counts
				})
