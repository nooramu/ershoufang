  ...
  import type { FormProps } from 'element-plus'
	...
	const changeUserPassword = async () => {
		if (passwordData.oldPassword && passwordData.newPassword) {
			...
			// 调用接口
			const res = await changePassword(localStorage.getItem('id') as unknown as number , passwordData.oldPassword, passwordData.newPassword)
			console.log(localStorage.getItem('id') as unknown as number )
			if (res.status == 0) {
				ElMessage({
					message: '修改成功',
					type: 'success',
